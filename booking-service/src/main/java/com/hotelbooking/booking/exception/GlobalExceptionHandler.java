...
public class GlobalExceptionHandler {
    ...
    /** Name of the PostgreSQL exclusion constraint installed by BookingConstraintInstaller. */
    ...
    public ResponseEntity<ApiErrorResponse> handleUnavailableDates(RoomNotAvailableException ex,
                                                                   HttpServletRequest request) {
    ...
    /**
     * room-service is down or its circuit is open. Retry-After tells well-behaved
     * clients how long to back off instead of hammering a struggling dependency.
     */
    @ExceptionHandler(CatalogUnavailableException.class)
    public ResponseEntity<ApiErrorResponse> handleCatalogDown(CatalogUnavailableException ex,
                                                              HttpServletRequest request) {
     ...
     *
     * <p>On PostgreSQL two simultaneous first-ever bookings for the same room can both
     * pass the application overlap check, and the exclusion constraint rejects the
     * loser at the database level. That surfaces here as an integrity violation, and
     * the guest should see the same friendly 409 as any other date clash — not a 500.
     */
    ...
    public ResponseEntity<ApiErrorResponse> handleDataIntegrity(DataIntegrityViolationException ex,
        ...
        if (detail.contains(OVERLAP_CONSTRAINT)) {
            log.info("Exclusion constraint rejected a concurrent overlapping booking on {}",
                    request.getRequestURI());
            return build(HttpStatus.CONFLICT,
                    "Those nights were just taken by another guest. Please choose different dates.",
                    request);
        }
    ...
    /** Two guests raced for the same room and the row lock timed out. */
    @ExceptionHandler({PessimisticLockingFailureException.class, CannotAcquireLockException.class})
    public ResponseEntity<ApiErrorResponse> handleLockTimeout(Exception ex,
                                                              HttpServletRequest request) {
        ...
        return ResponseEntity.status(HttpStatus.CONFLICT)
                ...
                .body(ApiErrorResponse.of(HttpStatus.CONFLICT.value(),
                        ...
                        "This room is being booked by someone else right now. Please try again.",
                        request.getRequestURI()));
    }
    ...
    public ResponseEntity<ApiErrorResponse> handleOptimisticLock(OptimisticLockingFailureException ex,
                                                                 HttpServletRequest request) {
        return build(HttpStatus.CONFLICT,
                "This booking was modified by someone else. Reload and try again.", request);
    }
    ...
    public ResponseEntity<ApiErrorResponse> handleValidation(MethodArgumentNotValidException ex,
        ...
        // Class-level rules such as the check-out-after-check-in assertion land here.
        ex.getBindingResult().getGlobalErrors()
                .forEach(error -> fieldErrors.putIfAbsent("dateRange", error.getDefaultMessage()));
    ...
    public ResponseEntity<ApiErrorResponse> handleUnreadable(HttpMessageNotReadableException ex,
                                                             HttpServletRequest request) {
        return build(HttpStatus.BAD_REQUEST,
                "Request body is missing or malformed (dates must be ISO yyyy-MM-dd)", request);
    }
